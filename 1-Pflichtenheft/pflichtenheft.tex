...
\textbf{Wähler}:
Person die bei einer Wahl eine bestimmte Partei, einen bestimmten Kandidaten o.Ä. wählt und die hierfür nötigen Berechtigungen hat.
...
\textbf{(Blockchain-)Ledger}:
Datenbank in einer Blockchain, welche alle Transaktionen, die auf der Blockchain ausgeführt wurden, enthält.

\textbf{(Blockchain-)Netzwerk}:
Menge an verknüpften Peers.
...
\textbf{Logdatei}:
Datei welche alle Ereignisse (z.B Informationen, Warnungen und Fehler) in einem System protokolliert.

\textbf{Linux}:
Beliebtes Unix-like Betriebssystem.

\textbf{Double-Spending-Problem}:
...
\textbf{Wahlleiter}:
Administrator der Wahl, legt die Bedingungen, Wähler und Kandidaten der Wahl fest.

\textbf{Konfiguration}:
Feste Belegung von einstellbaren Eigenschaften eines Systems.
