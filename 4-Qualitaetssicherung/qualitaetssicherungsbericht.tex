...
\teststep{}
		...
		\begin{enumerate}
			...
			\item Ein End- und Zeitpunkt muss definiert sein.
...
\teststep{}
		...
		{Die GUI zeigt den Zustand der Wahl an. Sabine Scholl hat eine Stimme und rechts ist ein Kreis angezeigt der in der Farbe von Sabine Scholl ist.}
	
\teststep{}
		{Drücke die Schaltfläche \enquote{Wahl endgültig beenden}.}
		{Es erscheint eine Meldung die darauf hinweist, dass dadurch die Wahl endgültig beendet wird und dieser Schritt nicht rückgängig gemacht werden kann. Das die Wahl neu gestartet wird und das Programm beendet.}
...
Wie im Entwurfsdokument beschrieben war es ursprünglich geplant die Kommunikation mit dem Blockchain-Netzwerk, ob die Wahl beendet ist, über die von Hyperledger-Fabric zu Verfügung gestellten Events zu implementieren Jedoch konnten wir diese nicht erfolgreich umsetzen, da ausgelöste Events nicht von dem Hyperledger-Fabric-SDK empfangen wurden. Daher wurde beschlossen wir auf polling zu wechseln. Dabei fragt der Klient regelmäßig das Blockchain-Netzwerk über Chaincdoe an, ob die Wahl geendet hat.
\\
Die dafür benötigten Änderungen beschränken sich auf das Model. Bei der Event-basierten Architektur gab es einen Listener dessen Interface von der SDKConnection definiert und vom Statemanagement implementiert wurde. Diese Listener-Funktionalität wurde komplett entfernt. Das hat zur Folge das die Beziehung zwischen der SDKConnection und dem Statemanagement komplett unidirektional ist. Außerdem wurde der Thread der zuvor regelmäßig neue Events in dem Blockchain-Netzwerk ausgelöst hatte umgeschrieben um regelmäßig nachzufragen ob die Wahl geendet hat.
\\
