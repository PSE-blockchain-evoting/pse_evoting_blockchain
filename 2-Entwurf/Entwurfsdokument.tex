...
\subsection{Interfaces zwischen den Paketen}
	...
	\item\fakeparagraph{Authentifizierung} Zuerst drueckt der Waehler den Knopf "Weiter", darauf wird actionPerformed() ausgerufen.
	Dann beginnt die Authorization durch Aufruf von authorization() auf einem VoterElection Objekt.
	Der Konstruktor von VoterSDKInterfaceImpl erhaelt zwei Parameter: Den Pfad zum Waehler-Zertifikat und listener.
	Die Control-Ebene braucht von der View-Ebene den Pfad zum Waehler-Zertifikat.
	Das passiert durch getAuthentication().
	listener ist Attribute von VoterElection.
	Mit der start Methode wurde neue Thread erzuegt. 
	Dann ein Waehler wird benachrichtigt, dass er erfolgreich authenifiziert wurde.
	Das passiert durch "true" Rueckgabe von VoterElection zu ActionListener.
	Dann an der View-Ebene von dem Waehler wurde seine Wahl angezeigt durch
	showChoice Methode zu VoterGUI. 
	In diesem Zeitpunkt passiert Panels-Switch von VoterAuthentication zu VoterChoice.
	Dazu wird neue Instance von VoterChoice-Panel erzuegt. 
